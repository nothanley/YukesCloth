...
WriteRCNIndexBuffer(std::stringstream& pBf, StSimMesh* sMesh, uint32_t numElements)
{
	...
	{
		uint8_t indexSize = sMesh->rcn.sizesA.at(i);
	...
	for (uint32_t i = 0; i < numElements; i++)
	{
		uint8_t width = sMesh->rcn.sizesA.at(i);

		for (uint8_t j = 0; j < width; j++)
		{
			uint32_t index = sMesh->rcn.values.at(itemIdx + j);
...
WriteRCNNormalBuffer(std::stringstream& pBf, StSimMesh* sMesh, uint32_t numElements)
{
	/* Write size values */
	for (uint32_t i = 0; i < numElements; i++) {
		uint8_t indexSize = sMesh->rcn.sizesB.at(i);
		WriteByte(pBf, indexSize);
	}
	AlignStream(pBf, 0x4);
	...
	for (uint32_t i = 0; i < numElements; i++)
	{
		uint8_t width = sMesh->rcn.sizesB.at(i);

		for (uint8_t j = 0; j < width; j++)
		{
			SkinVertex vert = sMesh->rcn.elements.at(itemIdx + j);
...
GetNodePalette(MeshSkin& skin, CSimObj* pSimObj, int& numWeights)
{
	...
	{
		if (weight.bones.size() > numWeights) {
			numWeights = weight.bones.size();
		}
...
WriteSkin(TagBuffer* pTagBuf, MeshSkin skin, std::vector<SimNode>& nodePalette, const int& weights)
{
	for (auto& blendWeight : skin.blendWeights)
		for (int i = 0; i < weights; i++) {
			uint32_t totalWeights = blendWeight.weights.size();
...
CClothEncoder::EncodeRCNData(TagBuffer* pTagBuf) {
	...
	uint32_t numChildren = pTagBuf->children.size();
	uint32_t numIdxs = sMesh->rcn.sizesA.size();
	uint32_t numVtxs = sMesh->rcn.sizesB.size();
...
CClothEncoder::EncodeSkinCalc(TagBuffer* pTagBuf) {
	...
	int matrixIdx = 0;
	for (auto& skinVtx : tag->pSimMesh->skinCalc) {
		/* Implement Vec4 search for matrix index */
		WriteUInt32(pTagBuf->binary, matrixIdx);
		matrixIdx++;
	}
	AlignStream(pTagBuf->binary, 16, 0xC);
}
...
CClothEncoder::EncodeLineDef(TagBuffer* pTagBuf) {
	...
	for (int i = 0; i < numLines; i++) {
		...
		uint32_t endIndex = FindNodeIndex(endNode.name, pSimMesh->nodePalette);
